...
cron.schedule('0 0 * * *', async () => {
  try {
    ...
    const expiredOffers = await Offer.find({ endDate: { $lt: currentDate.toDate() } });

    if (expiredOffers.length > 0) {
      const expiredOfferIds = expiredOffers.map(offer => offer._id);

      await Offer.deleteMany({ _id: { $in: expiredOfferIds } });
      ...
      for (const offer of expiredOffers) {
        ...
        if (offer.categories && offer.categories.length > 0) {
          query.category = { $in: offer.categories.map(category => category._id) };
        }
